...
Tingo 的 `contrib/` 是独立模块，提供中间件和工具组件，不进入核心依赖。
...
| 包 | 说明 |
|---|---|
| `contrib/cors` | 跨域资源共享中间件 |
| `contrib/secure` | 安全响应头（HSTS、X-Frame、XSS） |
| `contrib/csrf` | CSRF 防护（Cookie Double-Submit） |
| `contrib/jwt` | JWT 认证 |
...
| 包 | 说明 |
|---|---|
| `contrib/rate` | 令牌桶限流（内存） |
| `contrib/ratelimit` | 分布式滑动窗口限流（Redis） |
...
| `contrib/recovery` | panic 恢复中间件 |
| `contrib/gzip` | 响应压缩 |
| `contrib/logger` | HTTP 访问日志 |
| `contrib/static` | 静态文件服务 |
| `contrib/auth` | HTTP Basic / Bearer 鉴权 |
| `contrib/sessions` | 会话管理中间件 |

### 可观测性

| 包 | 说明 |
|---|---|
| `contrib/metric` | Prometheus 指标导出 |
| `contrib/trace` | TraceID 注入 + 慢日志 |
| `contrib/debug` | Whoops 风格错误调试页 |

### 配置

| 包 | 说明 |
|---|---|
| `contrib/registry` | 服务注册发现（file 后端 + 接口） |
...
~~~go
// go.mod 中添加
require github.com/xmszy/tingo/contrib v0.0.0
~~~
